...
export default {
  ...
  data(){ 
    return{
      ...
      dropzoneOptions: {
        url: document.querySelector('meta[name="base-url"]').getAttribute('content') + '/api/post-student-feeds', // 'https://httpbin.org/post',
        thumbnailWidth: 150,
        maxFilesize: 2,
        parallelUploads: 3,
        maxFiles: 10,
  ...
  methods: {
    ...
    displayTeacherFeeds(){
      axios.post(process.env.MIX_BASE_URL+'/display-student-feeds', { 'students_id' : this.user_id, 'feeds_id' : this.feeds_id }).then((res) => {
    ...
    postTeacherFeed: async function(){
      if (typeof this.$refs.myVueDropzone.processQueue() === 'undefined') {
        axios.post(process.env.MIX_BASE_URL+'/api/post-student-feeds', 
            { 'feed_body': this.post_msg, 'student_id': this.user_id 
        }).then((res) => {
          Swal.fire({
            position: 'top-end',
            icon: 'success',
